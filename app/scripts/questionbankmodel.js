 ...
 * The answer will be a list with zero or more elements
 * First check if the list was empty
 * Then extract the elements of the list 
 * remove square brackets ( [] ) and commas (,)
 * Then check if the student's answer matches the answer
...
QuestionBankModel.prototype.chooseQuestion = function(_firstQuestion, _lastQuestion) {
	...
	this.question += " If the answer is contains strings put the string elements in double quotes."
 ...
 * a better way.
 */
QuestionBankModel.prototype.setAnswers = function(_filter) {
	// Reset answers array
	this.answers = [];
	...
	if (this.questionIndex == 0) {
		this.answers.push(_filter.getFilterExpression());
